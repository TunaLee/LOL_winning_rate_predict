...
def predict(data):
    ...
    svm.fit(X,y)
    svm.predict(data)
    return svm.predict_proba([data])[1]

def deserializer(data):
    champ = pd.read_csv('Champ_kor.csv')
    teamid = data['teamId']
    our_team = []
    counter_team = []
    our_team += data['our_top_champ']
    our_team += data['our_jungle_champ']
    our_team += data['our_middle_champ']
    our_team += data['our_carry_champ']
    our_team += data['our_support_champ']
    counter_team += data['counter_top_champ']
    counter_team += data['counter_jungle_champ']
    counter_team += data['counter_middle_champ']
    counter_team += data['counter_carry_champ']
    counter_team += data['counter_support_champ']
        ...
        for i in range(len(champ)):
            if teamid == 100:
                if our_team[k] == champ.loc[:, 'number'][i]:
                    our_wr += champ[:, 'blue_WR'][i]
                    our_team_WR +=champ[:, 'blue_WR'][i]
                if counter_team[k] == champ.loc[:, 'number'][i]:
                    counter_wr += champ[:, 'red_WR'][i]
                    counter_team_WR += champ[:, 'red_WR'][i]
            else :
                if our_team[k] == champ.loc[:, 'number'][i]:
                    our_wr += champ[:, 'red_WR'][i]
                    our_team_WR +=champ[:, 'red_WR'][i]
                if counter_team[k] == champ.loc[:, 'number'][i]:
                    counter_wr += champ[:, 'blue_WR'][i]
                    counter_team_WR += champ[:, 'blue_WR'][i]
    if teamid == 200:
    ...
    try:
        our_top_win, our_top_loss, our_top_WR = crawler(data['our_top_summoner'], data['our_top_champ'])
    except:
        our_top_win, our_top_loss, our_top_WR = ['0W','0L',our_team_WR[0]]
    try:
        our_jungle_win, our_jungle_loss, our_jungle_WR = crawler(data['our_jungle_summoner'], data['our_jungle_champ'])
    except:
        our_jungle_win, our_jungle_loss, our_jungle_WR = ['0W','0L',our_team_WR[1]]
    try:
        our_middle_win, our_middle_loss, our_middle_WR = crawler(data['our_middle_summoner'], data['our_middle_champ'])
    except:
        our_middle_win, our_middle_loss, our_middle_WR = ['0W','0L',our_team_WR[2]]
    try:
        our_carry_win, our_carry_loss, our_carry_WR = crawler(data['our_carry_summoner'], data['our_carry_champ'])
    except:
        our_carry_win, our_carry_loss, our_carry_WR = ['0W','0L',our_team_WR[3]]
    try:
        our_support_win, our_support_loss, our_support_WR = crawler(data['our_support_summoner'], data['our_support_champ'])
    except:
    ...
    user_champ_WR = (our_top_WR + our_jungle_WR + our_middle_WR + our_carry_WR + our_support_WR)/5



    with open('LOL_Champ_Tier.json', 'rb') as f:
    ...
    for j in range(len(top_tier_df)):
        if our_team[0] == top_tier.loc[:, 'champid'][j]:
            top_tier = top_tier.loc[:, 'tier'][j]
        if counter_team[0] == top_tier.loc[:, 'champid'][j]:
            c_top_tier = top_tier.loc[:, 'tier'][j]

    for j in range(len(jungle_tier_df)):
        if our_team[1] == jungle_tier.loc[:, 'champid'][j]:
            jungle_tier = jungle_tier.loc[:, 'tier'][j]
        if counter_team[1] == jungle_tier.loc[:, 'champid'][j]:
            c_jungle_tier = jungle_tier.loc[:, 'tier'][j]
    ...
    with open('champion_id_and_name_list.json', 'rb') as f:
